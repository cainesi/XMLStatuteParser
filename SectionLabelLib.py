...
#roman numerals, i2n and n2i are dictionars for converting back and forth
numerals = [("m",1000),("cm",900),("d",500),("cd",400),("c",100),("xc",90),("l",50),("xl",40),("x",10),("ix",9),("v",5),("iv",4), ("i",1)]
def int2roman(number):
    if number < 0 or number > 5000:
        raise Exception("number out of range for numeral: %s"%str(number))
    l = []
    for c in numerals:
        while number >= c[1]:
            l.append(c[0])
...
#letters, a2i and i2a are dictionaries for converting back and forth
alphabet = [chr(c) for c in range(ord("a"),ord("z")+1)]
dalphabet = alphabet + ["".join(c) for c in zip(alphabet,alphabet)]
